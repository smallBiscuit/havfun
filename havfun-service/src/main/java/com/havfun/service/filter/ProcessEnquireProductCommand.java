...
import com.havfun.service.convertor.ProductAttributeOptionConvertor;
...
public class ProcessEnquireProductCommand implements Command {
	
	/** logger */
	private static final Logger LOGGER = LogManager.getLogger(ProcessEnquireProductCommand.class.getSimpleName());
	...
	public boolean execute(Context context) throws Exception {
		...
		EnquireProductRequest request = (EnquireProductRequest) context.get(AbstractHkListCoEventHandler.REQUEST);
		...
		if ( request.getProductId() == 0 ){
			...
			throw new HavfunException(ErrorCode.FAIL_TO_PROCESS, "productId is invalid");
		...
		if ( baseList != null && baseList.size() > 0 ){
			...
			List<ProductAttributeOption> optionList = productAttributeOptionDao.readByProductId( product.getProductId() );
			
			List<ProductAttributeOptionMessage> optionMessageList = new ArrayList<ProductAttributeOptionMessage>();
			
			Map<String, List<ProductAttributeOptionMessage>> attributeMap = new HashMap<String, List<ProductAttributeOptionMessage>>();
			
			for ( int i = 0;i < optionList.size(); i ++ ){
				
				ProductAttributeOption option = optionList.get(i);
				
				ProductAttributeOptionMessage message = ProductAttributeOptionConvertor.convertToMessage(option);
			
				optionMessageList.add(message);
				
			}

			attributeMap.put("attribute_pa_size", optionMessageList);
			
			baseMessage.setVariations(attributeMap);
			
			productMessage.setCustomizeProductBase(baseMessage);
			
		}
		
		EnquireProductResponse response = new EnquireProductResponse();
