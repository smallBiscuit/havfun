...
public class ProcessAdminUpdateMaterialCommand implements Command {
	
	/** logger */
	private static final Logger LOGGER = LogManager.getLogger(ProcessAdminUpdateMaterialCommand.class.getSimpleName());
	...
	public boolean execute(Context context) throws Exception {
		...
		AdminUpdateMaterialRequest request = (AdminUpdateMaterialRequest) context.get(AbstractHkListCoEventHandler.REQUEST);
		...
		int result = materialDao.update(material);
		
		if (result < 0 ) {
			throw new HavfunException(ErrorCode.FAIL_TO_PROCESS, "cannot update material");
		}
		
		AdminUpdateMaterialResponse response = new AdminUpdateMaterialResponse();
