package com.havfun.service.filter.admin;

import java.util.List;

import org.apache.commons.chain.Command;
import org.apache.commons.chain.Context;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.transaction.annotation.Propagation;
import org.springframework.transaction.annotation.Transactional;

import com.havfun.service.base.HavfunException;
import com.havfun.service.convertor.MaterialConvertor;
import com.havfun.service.dao.MaterialDao;
import com.havfun.service.entity.Material;
import com.havfun.service.handler.AbstractHkListCoEventHandler;
import com.havfun.service.message.admin.material.AdminEnquireMaterialRequest;
import com.havfun.service.message.admin.material.AdminEnquireMaterialResponse;
import com.havfun.service.message.constant.ErrorCode;
import com.havfun.service.message.data.MaterialMessage;

public class ProcessAdminEnquireMaterialCommand implements Command {
	
	/** logger */
	private static final Logger LOGGER = LogManager.getLogger(ProcessAdminEnquireMaterialCommand.class.getSimpleName());
	
	@Autowired
	private MaterialDao materialDao;
	
	@SuppressWarnings("unchecked")
	@Override
	@Transactional(value = "havfunTxnManager", propagation = Propagation.REQUIRED)
	public boolean execute(Context context) throws Exception {
		
		AdminEnquireMaterialRequest request = (AdminEnquireMaterialRequest) context.get(AbstractHkListCoEventHandler.REQUEST);
		
		if (request == null) {
			throw new HavfunException(ErrorCode.FAIL_TO_PROCESS, "request is null");
		}
		
		int materialId = request.getMaterialId();
		
		LOGGER.info("materialId: {}", materialId);
		
		Material material = materialDao.read(materialId);
		
		if (material == null) {
			throw new HavfunException(ErrorCode.FAIL_TO_PROCESS, "cannot read material");
		}
		
		MaterialMessage materialMessage = MaterialConvertor.convertToMessage(material);
		

		AdminEnquireMaterialResponse response = new AdminEnquireMaterialResponse();
		
		response.setResult(ErrorCode.NO_ERROR);
		
		response.setMaterialMessage(materialMessage);
		
		context.put(AbstractHkListCoEventHandler.RESPONSE, response);
		
		return false;
	}
	
	

}
