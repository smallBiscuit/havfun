...
public class GeoZoneDaoImpl implements GeoZoneDao {
	...
	private final static String GEO_ZONE_SELECT_SQL_BY_KEY = "select * from geoZone where geo_zeon_id = ?";

	private final static String GEO_ZONE_DELETE_SQL_BY_KEY = "delete from geoZone where geo_zeon_id = ?";

//	private final static String GEO_ZONE_SELECT_ALL_SQL = "select * from geoZone";

//	private final static String GEO_ZONE_SELECT_SQL_BY_GEO_ZEON_ID = "select * from geoZone where user_name = ?";
	...
	public Integer create(GeoZone newInstance) {
		...
		SimpleJdbcInsert simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName("geoZone").usingGeneratedKeyColumns("geo_zeon_id");
