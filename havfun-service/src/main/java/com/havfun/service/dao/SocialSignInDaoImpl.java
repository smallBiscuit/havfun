...
public class SocialSignInDaoImpl implements SocialSignInDao {
	...
	private final static String SOCIAL_SIGN_IN_UPDATE_SQL_BY_KEY = "update social_sign_in set  client_id = ?, email = ?, prvoider = ?, identifier = ?, register_timestamp = ?, last_visit_timestamp = ? where social_sign_in_id = ?";
	...
	private final static String SOCIAL_SIGN_IN_SELECT_SQL_BY_PROVIDER_IDENTIFIER = "select * from social_sign_in where social_sign_in_provider = ? and social_sign_in_identifier = ?";
	
//	private final static String SOCIAL_SIGN_IN_SELECT_ALL_SQL = "select * from socialSignIn";

//	private final static String SOCIAL_SIGN_IN_SELECT_SQL_BY_SOCIAL_SIGN_IN_ID = "select * from socialSignIn where user_name = ?";
	...
	public Integer create(SocialSignIn newInstance) {
		...
		parameters.put("provider", newInstance.getPrvoider());
		parameters.put("identifier", newInstance.getIdentifier());
		parameters.put("register_timestamp", new Timestamp( newInstance.getRegisterTimestamp() ) );
		parameters.put("last_visit_timestamp", new Timestamp( newInstance.getLastVisitTimestamp() ) );
	...
	public SocialSignIn readWithIdentifier(String provider, String identifier ) {
		...
		try {
			socialSignIn = jdbcTemplate.queryForObject(
					SOCIAL_SIGN_IN_SELECT_SQL_BY_PROVIDER_IDENTIFIER,
			rowMapper,
			new Object [] {
				provider, 
				identifier
	...
	public int update(SocialSignIn transientObject) {
		return jdbcTemplate.update(
				...
				new Object [] {
					...
					new Timestamp( transientObject.getRegisterTimestamp() ),
					new Timestamp( transientObject.getLastVisitTimestamp() ),
